...
The databricks CLI bundles top level is defined by file `my_mlops_project/bundle.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`my_mlops_project/databricks-resources/model-workflow-resource.yml`)
 - batch inference workflow (`my_mlops_project/databricks-resources/batch-inference-workflow-resource.yml`)
 - monitoring workflow (`my_mlops_project/databricks-resources/monitoring-workflow-resource.yml`)
 - feature engineering workflow (`my_mlops_project/databricks-resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`my_mlops_project/databricks-resources/ml-artifacts-resource.yml`)
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` environment is used by ML engineers to deploy ML resources to development workspace with `dev` environment configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-my-mlops-project-model     | /dev-my-mlops-project-experiment     |
| staging     | The `staging` environment is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` environment config.                                                             | staging workspace    | staging-my-mlops-project-model | /staging-my-mlops-project-experiment |
| prod        | The `prod` environment is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` environment config.                                                                      | prod workspace       | prod-my-mlops-project-model    | /prod-my-mlops-project-experiment    |
| test        | The `test` environment is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` environment. | staging workspace    | test-my-mlops-project-model    | /test-my-mlops-project-experiment    |
...
2. Complete the `TODO` in `my_mlops_project/bundle.yml` to add the dev workspace URI under `environments.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `my_mlops_project/databricks-resource/batch-inference-workflow-resource.yml` to pass
...
To get started, open `my_mlops_project/databricks-resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.environment}-my-mlops-project-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.environment}-my-mlops-project-batch-inference-job`
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.environment}-my-mlops-project-batch-inference-job`. 
